...
# load in data
calendar_df = fread('calendar.csv')
train_df = fread('sales_train_validation.csv')
price_df = fread('sell_prices.csv')
...
submission = forecast_df[sample, on=c('id')][, mget(names(forecast_df))]
